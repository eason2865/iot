...
<p align="center">
  ...
  <img alt="Release" src="https://img.shields.io/badge/Release-2.0.0-16a34a" />
</p>
...
- `core-rpc` 负责租户、设备、遥测、命令等核心业务，使用 gRPC + protobuf + etcd
- Kafka 作为遥测和命令的异步缓冲层
- `worker` 负责时序落库、业务状态更新和命令投递
- `admin` 使用 go-zero 提供租户、设备、遥测和命令的 REST API，并通过 gRPC 调用核心业务服务
...
- API 网关与核心业务分层：`admin` 作为 go-zero REST 网关，`core-rpc` 作为 gRPC 核心服务
- 服务发现：`core-rpc` 通过 etcd 注册与发现
