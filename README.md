...
<p align="center">
  ...
  <img alt="Release" src="https://img.shields.io/badge/Release-1.0.0-16a34a" />
</p>

Go + EMQX + Kafka + TDengine + PostgreSQL 的物联网平台骨架，面向设备接入、遥测采集、命令下发、状态查询和多租户管理。

当前版本：`1.0.0`

这不是一个只停留在“概念图”的示例仓库，而是一套已经把物联网关键闭环打通的开源基础设施：

- 设备通过 MQTT 直连 EMQX
- `ingress` 负责接入校验、标准化和事件解耦
- Kafka 作为遥测和命令的异步缓冲层
- `worker` 负责时序落库、业务状态更新和命令投递
- `admin` 提供租户、设备、遥测和命令的 REST API
- `demo` 模拟多租户多设备的随机请求流，适合压测和联调
...
- 标准契约：提供 OpenAPI、MQTT JSON Schema 和数据库迁移脚本
- 本地可运行：默认可以连接本机 Docker 的 PostgreSQL / Kafka / EMQX / TDengine

## 当前实现

- 4 个可启动入口：`cmd/admin`、`cmd/demo`、`cmd/ingress`、`cmd/worker`
- 10 组 HTTP 路由、13 个 API 操作
...
- Kubernetes + Helm：`admin` / `ingress` / `worker`
